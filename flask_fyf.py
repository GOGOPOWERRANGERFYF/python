...
def index():
    ...
    date = '上午 10:35'
    return render_template('index.html',context=context,date=date,list1=list1)#**关键字参数，传入参数后自动封装成一个字典dictionary，*关键字参数，传入元组tuple，传入参数后前端直接调用就行了，框架已经帮处理好了，不用担心数据类型的问题I
